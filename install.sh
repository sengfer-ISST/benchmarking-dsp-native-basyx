#!/usr/bin/env bash
# Bring up the dsp-native-basyx stack WITH monitoring on docker-compose, the same
# way benchmarking-EDC and benchmarking-factoryx-edc are launched, so the three
# connectors are benchmarked under one harness.
#
...
#   ./install.sh on         # identity ON  (full MVD/DCP: issuer-service + both hubs)
#   ./install.sh off        # identity OFF (mock) -> docker-compose-mock.monitoring.yaml
#   ./install.sh off --build  # mock arm, forcing an image rebuild
# Aliases: real|dcp|non-mock == on ; mock|no-id == off.
#
# Images are pulled NEVER (pull_policy: never) — build the connector image first
# with ./builddocker.sh. vault-init is a compose service, so vault seeding happens
# automatically on `up` (no separate ./rundocker.sh secret-deploy step needed).
set -euo pipefail
...
case "$MODE" in
  on|real|dcp|non-mock) COMPOSE_FILE="$REAL_COMPOSE"; LABEL="ON  (real MVD/DCP identity)"; IDMODE="on";;
  off|mock|no-id)       COMPOSE_FILE="$MOCK_COMPOSE"; LABEL="OFF (mock identity)";          IDMODE="off";;
esac
...
echo ">>> Starting dsp-native-basyx — identity ${LABEL} — ${COMPOSE_FILE} ..."
...
──────────────────────────────────────────────────────────────────────────────
dsp-native-basyx is up (docker-compose + monitoring).
  identity arm: ${LABEL}
...
Note: identity OFF runs the provider's MockValidationService and intentionally
drops the DCP consumer-controlplane (wire-incompatible) — drive that arm with the
plain-JSON consumer per docker-compose-mock.monitoring.yaml.
