...
//
// WHY THIS EXISTS: the "ON" deployment (docker-compose.monitoring.yaml) brings up
// the MVD/DCP identity stack (issuer-service + consumer/provider Identity Hubs +
// vault) but ships them EMPTY. Before any negotiation/transfer can succeed the
// dataspace must be provisioned: register the trusted issuer, announce the two
// members as holders, define the MembershipCredential, create each participant's
// identity, request their credentials, and store their STS secrets in the vault.
//
// That provisioning is exactly the fx-bruno/identities Bruno collection. Bruno is a
// desktop GUI — fine on a laptop, absent on the benchmark server. This script is a
// faithful headless port of that collection in k6 (the Bruno pre/post scripts are
// already JavaScript, so the base64/JSON token handling maps 1:1) and needs nothing
// beyond the k6 you already run the scenarios with. Run it ONCE right after
// `./install.sh on`, before the k6 scenarios:
//
//   k6 run k6-scripts/identity-setup.js                       # services on the docker host
//   k6 run -e IDHOST=10.0.0.5 k6-scripts/identity-setup.js    # driving from another host
//
